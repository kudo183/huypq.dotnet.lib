...
namespace huypq.QueryBuilder
{
    public class Paging
    {
        ...
        public static IQueryable<TSource> AddPaging<TSource>(
            IQueryable<TSource> query, ref int pageIndex, int pageSize, out int pageCount)
        {
            int itemCount = query.Count();

            query = AddPaging(query, ref pageIndex, itemCount, pageSize, out pageCount);

            return query;
        }

        public static IQueryable<TSource> AddPaging<TSource>(
            IQueryable<TSource> query, ref int pageIndex, int itemCount, int pageSize, out int pageCount)
        {
            pageCount = 0;
